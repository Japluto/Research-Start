...
- [C++ 5日速通](#c-5日速通)
  - [目录](#目录)
  - [Day 1 笔记](#day-1-笔记)
    - [一、从 C 到 C++ (C++新特征)](#一从-c-到-c-c新特征)
      - [核心知识点](#核心知识点)
      - [1. 输入输出流 (iostream) 与命名空间 (namespace)](#1-输入输出流-iostream-与命名空间-namespace)
      - [2. 引用 (Reference) - (C中没有，重点)](#2-引用-reference---c中没有重点)
      - [3. 动态内存 (new / delete)](#3-动态内存-new--delete)
      - [4. const (替代 #define)](#4-const-替代-define)
      - [5. 函数重载 (Overload)](#5-函数重载-overload)
      - [6. C++ 效率技巧 (来自 PPT 8)](#6-c-效率技巧-来自-ppt-8)
      - [实例与练习 (Day 1 - 模块一)](#实例与练习-day-1---模块一)
      - [练习1：C++ 版 `swap` 函数](#练习1c-版-swap-函数)
      - [练习1参考答案](#练习1参考答案)
    - [二、类的入门 (C++的灵魂)](#二类的入门-c的灵魂)
      - [核心知识点](#核心知识点-1)
      - [1. 什么是类 (Class)？](#1-什么是类-class)
      - [2. 封装 (Encapsulation) 与访问权限](#2-封装-encapsulation-与访问权限)
      - [3. 类的定义与实例化](#3-类的定义与实例化)
      - [4. 工程结构：接口与实现分离 (.h / .cpp)](#4-工程结构接口与实现分离-h--cpp)
      - [实例与练习 (Day 1 - 模块二)](#实例与练习-day-1---模块二)
      - [练习2：定义 `Box` 类](#练习2定义-box-类)
      - [练习2参考答案](#练习2参考答案)
  - [Day 2 笔记](#day-2-笔记)
    - [一、对象的“生” (构造函数)](#一对象的生-构造函数)
      - [核心知识点](#核心知识点-2)
      - [1. 什么是构造函数？](#1-什么是构造函数)
      - [2. 构造函数的规则](#2-构造函数的规则)
      - [3. 构造函数的类型 (考点)](#3-构造函数的类型-考点)
      - [实例与练习 (Day 2 - 模块一)](#实例与练习-day-2---模块一)
      - [练习1：升级 `Box` 类](#练习1升级-box-类)
      - [练习1参考答案](#练习1参考答案-1)
    - [二、对象的“死” (析构函数)](#二对象的死-析构函数)
      - [核心知识点](#核心知识点-3)
      - [1. 什么是析构函数？](#1-什么是析构函数)
      - [2. 析构函数的规则](#2-析构函数的规则)
      - [3. 调用时机 (考点)](#3-调用时机-考点)
      - [实例与练习 (Day 2 - 模块二)](#实例与练习-day-2---模块二)
      - [练习2：观察 `Box` 类的生命周期](#练习2观察-box-类的生命周期)
      - [练习2参考答案](#练习2参考答案-1)
    - [三、对象的“复制” (拷贝构造函数)](#三对象的复制-拷贝构造函数)
      - [核心知识点](#核心知识点-4)
      - [1. 什么是拷贝构造函数？](#1-什么是拷贝构造函数)
      - [2. 拷贝构造函数的格式 (必背！)](#2-拷贝构造函数的格式-必背)
      - [3. 调用时机 (必考！)](#3-调用时机-必考)
      - [4. 赋值 (`=`) vs 拷贝构造 (`=`)](#4-赋值--vs-拷贝构造-)
      - [实例与练习 (Day 2 - 模块三)](#实例与练习-day-2---模块三)
      - [练习3：拷贝构造函数探秘](#练习3拷贝构造函数探秘)
      - [练习3参考答案](#练习3参考答案)
    - [四、共享与保护 (static 与 const)](#四共享与保护-static-与-const)
      - [核心知识点](#核心知识点-5)
      - [1. `static` (静态/共享)](#1-static-静态共享)
      - [2. `const` (常量/保护)](#2-const-常量保护)
      - [实例与练习 (Day 2 - 模块四)](#实例与练习-day-2---模块四)
      - [练习4：`static` 与 `const` 综合应用](#练习4static-与-const-综合应用)
      - [练习4参考答案](#练习4参考答案)
    - [五、类的“朋友” (friend)](#五类的朋友-friend)
      - [核心知识点](#核心知识点-6)
      - [1. 友元函数](#1-友元函数)
      - [2. 友元类](#2-友元类)
      - [实例与练习 (Day 2 - 模块五)](#实例与练习-day-2---模块五)
      - [练习5：友元函数](#练习5友元函数)
      - [练习5参考答案](#练习5参考答案)
  - [Day 3 笔记](#day-3-笔记)
    - [一、运算符重载 (Operator Overloading)](#一运算符重载-operator-overloading)
      - [核心知识点](#核心知识点-7)
      - [1. 运算符重载的语法](#1-运算符重载的语法)
      - [2. 两种实现方式 (重点：成员 vs 友元)](#2-两种实现方式-重点成员-vs-友元)
      - [3. 运算符重载的规则 (考点)](#3-运算符重载的规则-考点)
      - [实例与练习 (Day 3 - 模块一)](#实例与练习-day-3---模块一)
      - [练习1：`Complex` 类加法重载](#练习1complex-类加法重载)
      - [练习1参考答案](#练习1参考答案-2)
    - [二、流插入(`<<`)与流提取(`>>`)重载](#二流插入与流提取重载)
      - [核心知识点](#核心知识点-8)
      - [1. 为什么必须是友元 (或非成员函数)？](#1-为什么必须是友元-或非成员函数)
      - [2. 签名 (必背)](#2-签名-必背)
      - [3. 链式调用 (Chainable)](#3-链式调用-chainable)
      - [实例与练习 (Day 3 - 模块二)](#实例与练习-day-3---模块二)
      - [练习2：为 `Complex` 类重载 `<<`](#练习2为-complex-类重载-)
      - [练习2参考答案](#练习2参考答案-2)
    - [三、文件输入输出 (I/O Streams)](#三文件输入输出-io-streams)
      - [核心知识点](#核心知识点-9)
      - [1. C++的I/O流概念](#1-c的io流概念)
      - [2. 文件I/O三(四)部曲 (文本文件)](#2-文件io三四部曲-文本文件)
      - [3. 文件打开模式 (File Modes)](#3-文件打开模式-file-modes)
      - [4. 二进制文件 (Binary I/O)](#4-二进制文件-binary-io)
      - [5. 格式化输出 (了解)](#5-格式化输出-了解)
      - [实例与练习 (Day 3 - 模块三)](#实例与练习-day-3---模块三)
      - [练习3：文本文件读写](#练习3文本文件读写)
      - [练习3参考答案](#练习3参考答案-1)
      - [练习4：二进制文件读写 `struct`](#练习4二进制文件读写-struct)
      - [练习4参考答案](#练习4参考答案-1)
  - [Day 4 笔记](#day-4-笔记)
    - [一、什么是继承 (Inheritance)？](#一什么是继承-inheritance)
      - [核心知识点](#核心知识点-10)
      - [1. 定义](#1-定义)
      - [2. 派生类的声明 (继承语法)](#2-派生类的声明-继承语法)
      - [3. 继承方式 (访问控制) (核心考点)](#3-继承方式-访问控制-核心考点)
    - [二、派生类的构造与析构 (核心考点)](#二派生类的构造与析构-核心考点)
      - [核心知识点](#核心知识点-11)
      - [1. 派生类的构造函数 (重点)](#1-派生类的构造函数-重点)
      - [2. 派生类的析构函数](#2-派生类的析构函数)
      - [实例与练习 (Day 4 - 模块一)](#实例与练习-day-4---模块一)
      - [练习1：简单的公有继承](#练习1简单的公有继承)
      - [练习1参考答案](#练习1参考答案-3)
    - [三、多重继承与二义性](#三多重继承与二义性)
      - [核心知识点](#核心知识点-12)
      - [1. 多重继承 (Multiple Inheritance)](#1-多重继承-multiple-inheritance)
      - [2. 二义性问题 (Ambiguity)](#2-二义性问题-ambiguity)
    - [四、虚基类 (Virtual Base Class)](#四虚基类-virtual-base-class)
      - [核心知识点](#核心知识点-13)
      - [1. 菱形继承问题 (The Diamond Problem)](#1-菱形继承问题-the-diamond-problem)
      - [2. 虚基类 (Virtual Base Class) 解决方案](#2-虚基类-virtual-base-class-解决方案)
      - [3. 虚基类的构造 (考点)](#3-虚基类的构造-考点)
      - [实例与练习 (Day 4 - 模块二)](#实例与练习-day-4---模块二)
      - [练习2：菱形继承与虚基类](#练习2菱形继承与虚基类)
      - [练习2参考答案](#练习2参考答案-3)
  - [Day 5 笔记](#day-5-笔记)
    - [一、多态性 (Polymorphism)](#一多态性-polymorphism)
      - [核心知识点](#核心知识点-14)
      - [1. 静态联编 (Static Binding) vs 动态联编 (Dynamic Binding)](#1-静态联编-static-binding-vs-动态联编-dynamic-binding)
      - [2. 虚函数 (Virtual Functions) (核心考点)](#2-虚函数-virtual-functions-核心考点)
      - [3. 虚析构函数 (Virtual Destructor) (必考！)](#3-虚析构函数-virtual-destructor-必考)
      - [4. 纯虚函数与抽象类 (Pure Virtual \& ABC)](#4-纯虚函数与抽象类-pure-virtual--abc)
      - [实例与练习 (Day 5 - 模块一)](#实例与练习-day-5---模块一)
      - [练习1：多态与抽象类](#练习1多态与抽象类)
      - [练习1参考答案](#练习1参考答案-4)
    - [二、编程规范与总复习 (PPT 8)](#二编程规范与总复习-ppt-8)
      - [核心知识点 (编程规范)](#核心知识点-编程规范)
      - [实例与练习 (Day 5 - 模块二)](#实例与练习-day-5---模块二)
      - [练习2：代码重构 (Refactor)](#练习2代码重构-refactor)
      - [练习2参考答案 (重构后的代码)](#练习2参考答案-重构后的代码)
    - [看到这里恭喜你！](#看到这里恭喜你)
